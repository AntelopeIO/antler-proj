...
namespace antler {
   struct init_project {
      inline init_project(CLI::App& app) {
         ...
         subcommand->add_option("-n, --name", name, "The name of the project. If --path is not specified, name is used.")->required();
      ...
      int32_t exec() {
         ...
         antler::project::project proj = {proj_path, name, version};

         if (!proj.init_dirs(proj_path))
            return -1;

         proj.sync();

         system::info_log("Created project {0} at location {1}", name, proj_path);
         return 0;
      }
