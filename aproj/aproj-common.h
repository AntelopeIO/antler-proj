...
#define RETURN_USAGE(X) { std::stringstream ss; ss X; return usage(ss.str()); }
...
inline void dump_obj_deps(const T& obj_list, bool app, bool lib, bool tst, std::ostream& os=std::cout) {
   ...
   for(const auto& a : obj_list) {
      ...
      if(list.empty())
         continue;
      switch(a.type()) {
         case antler::project::object::type_t::app: if(!app) { continue; } break;
         case antler::project::object::type_t::lib: if(!lib) { continue; } break;
         case antler::project::object::type_t::test: if(!tst) { continue; } break;
