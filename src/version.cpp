...
int64_t version::raw_compare(std::string_view o) const noexcept {
   std::vector<std::string> l;
   boost::split(l, m_raw, boost::is_any_of(".,-+;"));

   std::vector<std::string> r;
   boost::split(r, o, boost::is_any_of(".,-+;"));
   ...
   for (size_t i = 0; i < std::min(l.size(), r.size()); ++i) {
      ...
      if (ln != std::string_view::npos || rn != std::string_view::npos) {
         ...
         if (ln == std::string_view::npos) {
            (void)string::from(l[i], lnum);
            lremain = l[i];
         }
         else {
            (void)string::from(l[i].substr(0, ln), lnum);
         ...
         if (rn == std::string_view::npos) {
            (void)string::from(r[i], rnum);
            rremain = r[i];
         }
         else {
            (void)string::from(r[i].substr(0, rn), rnum);
